...
export const AVAILABLE_TOOLS: ToolDefinition[] = [
  ...
  },
  {
    ...
    description: 'Execute Python 3.12 code in a sandboxed on-device interpreter and return stdout, stderr, and the value of the last expression. numpy, pandas, and matplotlib are preinstalled and run fully offline. To use other packages, list them in "packages" and they install from PyPI before the code runs (needs network). The interpreter is a WebAssembly sandbox with no operating-system processes, shell, or network sockets: subprocess, os.system, pip run from code, git, and requests or urllib all fail (subprocess raises "emscripten does not support processes"). To bring in a remote file, script, or skill, call the read_url tool to fetch its raw contents (use a raw file URL, not a repository page), then write_file it into the workspace and import or exec it - never git clone or shell out. To show a plot, build a figure with matplotlib (import matplotlib.pyplot as plt; plt.plot(...)); it is captured automatically and shown to the user in the chat, so you do not need savefig or plt.show. Do not draw charts by printing HTML, SVG, or <img> data URIs - those are shown as raw text, not rendered; only matplotlib figures become images. Write a complete script and print() what you need to see. Variables persist between calls in the same session.',
    icon: 'terminal',
